...
sub main { # {{{
    ...
    assert_hashref($result, 'Response is a hashref');
    ...
    assume_exists(\%pass, $response->{'response'}, 'Response is supported');

    my $body = $pass{ $response->{'response'} }->($response);
...
sub pass_CONTENT { # {{{
    ...
    assume_defined($result->{'mime_type'}, q{Result's mime-type is defined.});
    assume_defined($result->{'content'},   q{Content is defined.});
...
sub pass_FILE { # {{{
    ...
    assume_defined($result->{'mime_type'}, q{Result's mime-type is defined.});
    assume_defined($result->{'path'},      q{File's path is defined.});
