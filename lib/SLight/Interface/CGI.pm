...
sub main { # {{{
    ...
    if ($result->{'send_file'}) {
        # Instead of serving the content generated by the Handler,
        # browser should receive a file, from HDD.
        # Attachments are sent using this way.

        $self->set_header(
            name  => "content-type",
            value => $result->{'send_file'}->{'mime_type'}
        );

        # Overwrite 'content', if there was a 'path' given.
        if ($result->{'send_file'}->{'path'}) {
            $result->{'content'} = read_file($result->{'send_file'}->{'path'});
        }
        elsif ($result->{'send_file'}->{'data'}) {
            $result->{'content'} = $result->{'send_file'}->{'data'};
        }
    }
        ...
        my %content_types = (
            THT  => 'text/html; character-set: UTF-8',
            GHT  => 'text/html; character-set: UTF-8',
            JSON => 'text/json; character-set: UTF-8',
            YAML => 'text/yaml; character-set: UTF-8',
            XML  => 'text/xml; character-set: UTF-8',
        );

        $self->set_header(
            name  => "content-type",
            value => ( $content_types{$output_type} or 'application/octet-stream' )
        );
    }

#    my $returned = $self->headers_string() .qq{\n} . ( $result->{'content'} or q{} );
#
#    use File::Slurp;
#    write_file('/tmp/test.bin', $returned);    
#
#    return $returned;
...
sub safe_main { # {{{
    ...
    if ($cgi->truncated) {
        return (internal_error_response("Form data has been truncated."), 'THT');
    }
...
#    use Data::Dumper; print Dumper $url_hash;
    ...
    if ($self->{'CGI'}->{'got_cookies'}->{$session_cookie}) {
        SLight::Core::Session::restore($self->{'CGI'}->{'got_cookies'}->{$session_cookie}->{'value'});
    }
    else {
        SLight::Core::Session::start();
    }

    $output_type = delete $url_hash->{'output'};
    delete $url_hash->{'theme'};
    ...
    return ( $request_result, $output_type );
} # }}}
